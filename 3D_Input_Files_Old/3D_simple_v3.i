...
##Trelis geometry
[Mesh]
	file = rmctests/3D/3D_simplecoarse.e
...
[AuxKernels]
	...
	[../]
	[./gfp_aux] ##solves for gaseous fission products
		type = GaseousProductsAux
		variable = gaseous_products
		temp = temp
		burnup = burnup
		burnup_dt = burnup_dt
		execute_on = timestep
		model_GFP = true
	[../]
	[./sfp_aux] ##solves for solid fission products
		type = SolidProductsAux
		variable = solid_products
		burnup = burnup
		execute_on = timestep
		model_SFP = true
	[../]
	[./alpha_aux] ##solves for solid fission products
		type = AlphaAux
		variable = alpha
		temp = temp
		execute_on = timestep
...
[Materials]
    ...
    type = PelletThermalMaterial
		...
		gaseous_products = gaseous_products
		alpha1 = alpha
		block = pellets
  ...
  [./Mechanical_Pellets] ##Determines the solid mechanical properties of the pellet - stresses, strains
  	...
  	disp_z = disp_z
		...
		stress_free_temp = 300
		model_thermal_expansion = true
		display_values = false
	[../]
	[./YM_Pellets]
		type = PelletYMMaterial
		temp = temp
		block = pellets
		disp_x = disp_x
		disp_y = disp_y
