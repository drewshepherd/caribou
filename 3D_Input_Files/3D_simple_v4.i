###################################################
##3D with Q_fission heat generation, porosity and burnup-dependent heat conduction,
##Diffusional creep on, thermal expansion on only for sheath
###################################################
##***********contact causes a segmentation fault
##diffusional creep does not work
##pressure is off
...
##Trelis geometry
[Mesh]
	file = 3D_Geometries/3D_simple_v1.e
...
[AuxVariables]
	...
	[./vstrain] ##solves for the volumetric strain
		...
		block = pellets
	[../]
	[./k_pellets] ##solves for the local burnup - used everywhere except FissionHeatMaterial
		order = FIRST 
		family = MONOMIAL
		block = pellets
	[../]
	[./k_pellets_dT] ##solves for the local burnup_dt
		order = FIRST 
		family = MONOMIAL
		block = pellets
	[../]
	[./k_sheath] ##solves for the local burnup - used everywhere except FissionHeatMaterial
		order = FIRST 
		family = MONOMIAL
		block = sheath
	[../]
	[./k_sheath_dT] ##solves for the local burnup_dt
		order = FIRST 
		family = MONOMIAL
		block = sheath
	[../]
...
[Outputs]
  file_base = 3D_Paraview/3D_simple_final_out
  output_initial = true
  exodus = true
